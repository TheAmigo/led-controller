...
def fadeColor(String color, BigDecimal fadetime=1) {
    writeLog("fadeColor($color, $fadetime)")
    sendCmd('{"cmd": "color", "color": "' + color + '", "duration": ' + fadetime + '}')
}

def setColor(Map hsv) {
    writeLog("setColor($hsv)")
    // Rename level to value and pass it as HSV
    sendCmd('{"cmd": "hsv", "hue": ' + hsv['hue'] + ', "saturation": ' + hsv['saturation'] + ', "value": ' + hsv['level'] + '}')
}

def setHue(float hue) {
    writeLog("setHue($hue)")
    sendCmd('{"cmd": "set_hue", "hue": ' + hue + '}')
}

def setSaturation(float sat) {
    writeLog("setHue($sat)")
    sendCmd('{"cmd": "set_sat", "saturation": ' + sat + '}')
}
